...
public class RecursiveMazeGenerator extends MazeGenerator {
    ...
    private boolean isBlocked(Vector2 pos) {
        if (!board.isInBounds(pos)) {
            return true;   //out of bounds -> no passage
        } else {
    ...
    /**
     * Checks if the wall doesn't block an existing passage before or after it
     *
...
//        System.out.println("isPassage before and after "+isPassage(posBeforeWall) + " "+ isPassage(posAfterWall));
        return isBlocked(posBeforeWall) && isBlocked(posAfterWall);
    }
    ...
    private void bulidWall(Vector2 wallStart, Vector2 diff, int wallLength) {
        ...
        for (int i = 0; i < wallLength; i++) {
            if (i != passageIndex) {
                wallsToBulid.add(wallStart.copy());
            }
            wallStart.add(diff);
        }
    }
...
//        System.out.println("top left, bottom right, isVertical "+isVertical+" "+topLeft+" "+bottomRight);
        ...
        boolean failedToBuildTheWall = false;   //tried to build a wall but failed because of the passage
